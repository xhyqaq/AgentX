...
@Service
public class PluginManagerDomainService extends PluginBaseDoaminService {
     ...
     * 更新插件
     */
    public PluginEntity updatePlugin(String id, String name, String description, String repositoryUrl, String license) {
        PluginEntity plugin = pluginRepository.selectById(id);
        if (plugin == null) {
            throw new BusinessException("插件不存在");
        }

        plugin.update(name, description, GitRepository.from(repositoryUrl), License.from(license));
        pluginRepository.updateById(plugin);
        return plugin;
    }
}
